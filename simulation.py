...
class Simulator(object):
    ...
    @execution_step
    def assert_attr_regexp(self, selector, attr, regexp):
        node_attr_value = self._get_attr_with_period(self.assert_exists(selector), attr)
        assert re.match(regexp, node_attr_value), "%s.%s value %s does not match %s regexp" % (selector, attr, node_attr_value, regexp)
